...
func init() {
	...
	utilruntime.Must(clavexv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&clavexServerURL, "clavex-server-url", os.Getenv("CLAVEX_SERVER_URL"),
		"Base URL of the Clavex Admin API this operator reconciles against "+
			"(e.g. https://id.acme.example). Required. Can also be set via CLAVEX_SERVER_URL.")
	opts := zap.Options{
		Development: true,
	}
	...
	if clavexServerURL == "" {
		setupLog.Error(fmt.Errorf("missing required configuration"), "--clavex-server-url (or CLAVEX_SERVER_URL) is required")
		os.Exit(1)
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "6d0c9dea.clavex.eu",
	...
	if err := (&controller.ClavexIdentityProviderReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if err := (&controller.ClavexRoleReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if err := (&controller.ClavexGroupReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if err := (&controller.ClavexWebhookReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if err := (&controller.ClavexOrgReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if err := (&controller.ClavexAuthPolicyReconciler{
		Client:          mgr.GetClient(),
		Scheme:          mgr.GetScheme(),
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupClavexClientWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "Failed to create webhook", "webhook", "ClavexClient")
			os.Exit(1)
		}
	}
